...
public class ArtikelDialog {
  ...
  public void readInput() {
    try (final Scanner scanner = new Scanner(System.in)) {
      while (true) {
        ...
        if (artikel == null
            && !input.equalsIgnoreCase("erstellen")
    ...
    try {
      switch (input) {
        case "erstellen":
          if (artikel != null) {
            final String mode =
                readString(scanner, "Soll der aktuelle Artikel überschrieben werden? (Ja/Nein)")
                    .toLowerCase();
  ...
  private Artikel createArtikel(Scanner scanner) {
    final String constructor =
        readString(scanner, "Soll der Konstruktor mit allen Paramtern genutzt werden? (Ja/Nein): ");
